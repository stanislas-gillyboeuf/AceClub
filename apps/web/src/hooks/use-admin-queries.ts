...
export function useOrganizationMembers(params: ListOrganizationMembersParams) {
  const searchParams = new URLSearchParams();
...
export function useOrganizationInvitations(params: ListOrganizationInvitationsParams) {
  const searchParams = new URLSearchParams();
  searchParams.set("organizationId", params.organizationId);
  const qs = searchParams.toString();
...
export function useFeatureFlags() {
  return useQuery({
    ...
    queryFn: () => apiClient<ListFeatureFlagsResponse>("/admin/feature-flags"),
  });
}
