...
import { toggleRuleValidator } from "../validators";

export const toggleRule = async (c: Context<HonoContext>) => {
  const currentUser = c.get("user")!;
  // @ts-ignore
  const validated = c.req.valid("json") as z.infer<typeof toggleRuleValidator>;
  ...
  await logGridAudit({
    ...
    action: "updated",
    summary: `Règle "${updated.name}" ${validated.isActive ? "activée" : "désactivée"}`,
  });
