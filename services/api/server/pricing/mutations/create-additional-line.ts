...
import { createAdditionalLineValidator } from "../validators";

export const createAdditionalLine = async (c: Context<HonoContext>) => {
  const currentUser = c.get("user")!;
  // @ts-ignore
  const validated = c.req.valid("json") as z.infer<typeof createAdditionalLineValidator>;
  ...
  await logGridAudit({
    ...
    action: "updated",
    summary: `Ligne additionnelle "${validated.name}" créée`,
  });

  return c.json({ gridId, additionalLine: created }, 201);
};
