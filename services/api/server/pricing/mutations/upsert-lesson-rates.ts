...
import { db } from "../../../db";
import { tarifGrid, tarifLessonRate } from "../../../db/schema";
import { assertClubFullAdmin } from "../../../middleware/club-admin";
import { getOrCreateEditableVersion } from "../lib/versioning";
import { logGridAudit } from "../lib/audit";
import { upsertLessonRatesValidator } from "../validators";

// Delete-then-insert is safe here (unlike age categories): nothing references tarifLessonRate.id.
export const upsertLessonRates = async (c: Context<HonoContext>) => {
  const currentUser = c.get("user")!;
  // @ts-ignore
  const validated = c.req.valid("json") as z.infer<typeof upsertLessonRatesValidator>;
  ...
  const rates = await db.transaction(async (tx) => {
    await tx.delete(tarifLessonRate).where(eq(tarifLessonRate.tarifGridId, gridId));
    ...
    return tx.select().from(tarifLessonRate).where(eq(tarifLessonRate.tarifGridId, gridId));
  });
  ...
  await logGridAudit({
    ...
    action: "updated",
    summary: "Tarifs de cours modifiés",
  });

  return c.json({ gridId, lessonRates: rates });
};
